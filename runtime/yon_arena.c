...
#include "leech_orbits.h"   /* yon_leech_m24_orbit_pure */
...
/* The orbit of a point is the pure M24 orbit, from the shared leech_orbits
 * module — the single source of truth, the same the runtime primitive reads. */
static uint32_t arena_m24_orbit(yon_xcoord_t point) {
    return yon_leech_m24_orbit_pure(point);
}
