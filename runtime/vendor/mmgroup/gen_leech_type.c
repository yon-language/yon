...
/*************************************************************************
***  Fast and coarse version of function gen_leech2_subtype
*************************************************************************/

/**
  @brief A fast, but coarser version of function gen_leech2_subtype
...
{
    return gen_leech2_coarse_subtype_inline(v2);
}
...
{
    uint_fast32_t theta, syn, scalar, w, coc, cw, octad, lsb, ccv;
    ...
    coc = (v2 ^ theta) & 0x7ff;
    // Put cw = 1 if coc has weight 2 and cw = 0 otherwise
    cw = MAT24_SYNDROME_TABLE[coc & 0x7ff] >> 15;
    if (cw == w) return 4;
    // Here v2 corresponds to a (possibly complemented) octad.
    // Let ``octad`` be that non-complemented octad in vector rep.
    v2 ^= (1 - w) << 23;
    octad = mat24_def_gcode_to_vect(v2 >> 12); 
    // Put ccw = cocode word of v (in vector rep), such that
    // the cocode word is a suboctad of the octad if possible.
    lsb = mat24_def_lsbit24(octad);
    coc ^= MAT24_RECIP_BASIS[lsb];
    syn = MAT24_SYNDROME_TABLE[coc & 0x7ff];
    ccv = mat24_def_syndrome_from_table(syn) ^ (1UL << lsb);
    // Type is 2 if ccv is a subset of octad and 4 otherwise.
    return ((octad & ccv) ^ ccv) ? 4 : 2;
}
