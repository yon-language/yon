...
const FIELDS = [
  ...
  {key: 'biometrics', label: 'heart_rate / biometrics', science: true, security: false, legal: false},
  {key: 'containment', label: 'containment_status', science: false, security: true, legal: false},
  {key: 'individual_id', label: 'individual_id', science: true, security: true, legal: 'quotiented'},
];

const LENSES = [
  {id: 'science', name: 'Science', note: 'Sees the animal in full: genome, biometrics, position.'},
  {id: 'security', name: 'Security', note: 'Sees position, containment, identity — never the biometrics.'},
  {id: 'legal', name: 'Legal', note: 'Works in PublicPark = Park / cohort. Identity is not hidden — it cannot be named.'},
];
...
export default function ThreeLenses() {
  ...
  return (
    <div style={{border: '1px solid var(--ifm-color-emphasis-300)', borderRadius: 12, padding: '1rem 1.25rem', margin: '1.5rem 0'}}>
          ...
          {FIELDS.map((f) => {
            ...
            return (
              <tr key={f.key} style={{borderTop: '1px solid var(--ifm-color-emphasis-200)'}}>
                <td style={{padding: '6px 8px', fontFamily: 'var(--ifm-font-family-monospace)'}}>{f.label}</td>
                <td style={{padding: '6px 8px', textAlign: 'right', color: tone[c.tone], fontWeight: 500}}>{c.text}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <p style={{margin: '12px 0 0', fontSize: 13, color: 'var(--ifm-color-content-secondary)'}}>
        ...
        <code> show id = individual_id</code> would not factor through the quotient — it does not compile.
      </p>
    </div>
  );
}
