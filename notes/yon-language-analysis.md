...
- **Verifica al kernel** [PARZIALE]: `core_wf` — il checker dipendente ri-certifica i tipi del Core lowerato (disciplina "il kernel ri-verifica l'elaboratore"), gate hard sui tipi, advisory sui termini. Cattura errori che la superficie lascia passare (es. `El` di un non-codice).
