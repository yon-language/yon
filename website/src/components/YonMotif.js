...
function renderMotif(container, opts) {
  ...
  const W = 1200, H = 820;
  const C = { x: 852, y: 404 }, R = 170;
  const rand = rng(20240606);

  if (!document.getElementById('yon-motif-kf')) {
    const st = document.createElement('style');
    st.id = 'yon-motif-kf';
    st.textContent = '@keyframes yon-draw{to{stroke-dashoffset:0}}@keyframes yon-fade{to{opacity:1}}';
  ...
  const gField = el('g', {}), gArrows = el('g', {}), gHeads = el('g', {});
    ...
    for (let j = i + 1; j < dots.length; j++) {
      ...
      if (d < 96 && rand() < 0.22) {
        gField.appendChild(el('line', {
          x1: dots[i].x, y1: dots[i].y, x2: dots[j].x, y2: dots[j].y,
          stroke: '#6A5FC0', 'stroke-width': 1, 'stroke-opacity': 0.12 }));
      }
    }
  }
  dots.forEach((d) => {
    gField.appendChild(el('circle', {
      cx: d.x, cy: d.y, r: 1.2 + rand() * 1.7,
      fill: '#A59ED7', 'fill-opacity': (0.16 + rand() * 0.34).toFixed(2) }));
  });

  if (enso) {
    const a0 = -1.15, a1 = Math.PI * 2 - 0.5;
    const sx = C.x + R * Math.cos(a0), sy = C.y + R * Math.sin(a0);
    const ex = C.x + R * Math.cos(a1), ey = C.y + R * Math.sin(a1);
    gField.appendChild(el('path', {
      d: `M ${sx} ${sy} A ${R} ${R} 0 1 1 ${ex} ${ey}`,
      stroke: '#6A5FC0', 'stroke-width': 1.4, 'stroke-opacity': 0.4, 'stroke-linecap': 'round' }));
  }
  ...
  for (let i = 0; i < M; i++) {
    ...
    const c1 = { x: S.x - u.x * (reach - R) * 0.42 + perp.x * bow,
  ...
  svg.appendChild(gField); svg.appendChild(gArrows); svg.appendChild(gHeads);
  ...
  drawables.forEach(({ path, head, idx }) => {
    ...
    setTimeout(() => { path.style.strokeDashoffset = '0'; head.style.opacity = '1'; }, delay + 1250);
  });

  const sq = el('g', { 'stroke-linecap': 'round' });
  const x0 = 70, y0 = 612, s = 86;
  sq.appendChild(el('path', {
    d: `M ${x0 + 10} ${y0} L ${x0 + s} ${y0} L ${x0 + s} ${y0 + s} L ${x0} ${y0 + s} L ${x0} ${y0 + 12}`,
    stroke: '#6A5FC0', 'stroke-width': 1.4, 'stroke-opacity': 0.5 }));
  sq.appendChild(el('path', {
    d: `M ${x0 + 8} ${y0 + 8} L ${x0 + 20} ${y0 + 8} M ${x0 + 8} ${y0 + 8} L ${x0 + 8} ${y0 + 20}`,
    stroke: '#A59ED7', 'stroke-width': 1.4, 'stroke-opacity': 0.6 }));
  svg.appendChild(sq);
}
