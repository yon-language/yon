...
//
//     !topos.section<P>      -> i32 (yon_xcoord_t XLeech2 encoding)
...
// Unrecognized types produce 0 and signal an error.
...
computePlaceLayouts(ModuleOp module) {
  ...
  module.walk([&](PlaceOp place) {
    ...
    for (Operation &nested : place.getBody().front()) {
      ...
      if (size == 0) {
        // Unsupported type: emit a warning via emitOpError. Continue with
        // alignment 8 so as not to block the layout (it will be reported during
        // the lowering of the section).
        size = 8;
      }
...
// of a first parameter `instance: memref<N x i8>` corresponding to the
// enclosing place.
...
// inside a world or place). The body optionally contains the transformation of
// the fields. Lowering:
//
//   topos.move @M from @Src to @Dst { body }
//   ->
//   func.func @M__move(%src: memref<Ns x i8>) -> memref<Nd x i8> {
//     // converted body; if the body is empty -> identity (memref.copy)
//   }
//
...
// The xcoord flows directly into the per-move function; the runtime
// dereferences it via yon_xheap_apply_move inside the wrapper.
...
//   ->
//   %v = func.call @P__get_fname(%sec) : (!llvm.ptr) -> ResultTy
//
// The runtime function `P__get_fname` is generated as a private declaration
// with an identity-zero body (returns the result type's zero constant) until
// cross-space access to the section's real payload is closed. The scheme
// mirrors LowerMoveOp: a declaration with a default implementation that the
// XLeech2 backend can replace via symbol resolution.
